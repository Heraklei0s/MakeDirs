...
## This function standardizes input and improves readability
def slugify(string):
...
def get_root():
    root = pathlib.PurePath(
        input("What's the full path you'd like for the project?")
    )
...
def check_delete_root(root):
    if os.path.exists(root):
        ...
        try:
            delete = strtobool(input("Delete existing files or directories? [y/n] ").lower())
        except ValueError:
